...
export const invoiceSlice = createSlice({
    ...
    reducers: {
        ...
        updateInvoiceDraft: (
            state,
            action: PayloadAction<Partial<Invoice>>
        ) => {
        ...
        saveInvoice: (state, action: PayloadAction<Partial<Invoice>>) => {
            const targetIndex = state.invoices.findIndex(
        ...
        permanentDeleteInvoice: (state, action: PayloadAction<string>) => {
            const targetIndex = state.trashedInvoices.findIndex(
                (invoice) => invoice.id === action.payload
            )
    ...
    if (searchText) {
        filteredInvoices = filteredInvoices.filter(
            (invoice) =>
                ...
                invoice.customerReading
                    .toLowerCase()
                    .includes(customerNameReading) ||
